import json, pymysql,jieba, pygal
from wordcloud import WordCloud


class Comment(object):
    def __init__(self):
        self.conn = pymysql.connect(user='root', password='123456', db='test')
    ...
    def save_db(self, comment_info):
        """保存到数据库"""
        self.cursor.execute("""CREATE TABLE IF NOT EXISTS taobao1
        (
        ...
        for i in comment_info:
            self.cursor.execute("SELECT comment_id FROM taobao1 WHERE comment_id=%s" % i['comment_id'])
            q = self.cursor.fetchone()
            if q == None:  # 判断是否以存入
                try:
                    self.cursor.execute("INSERT INTO taobao1(comment_id,comment,create_date,append_comment,action_sku) VALUES (%s,%s,%s,%s,%s)", [i['comment_id'], i['评论'], i['评论时间'], i['追加评论'], i['商品类型']])
                except:
                    # 由于编码问题，可能会出错，所以需要修改存储字段为utf8mb4编码。
                    self.cursor.execute('ALTER TABLE taobao1 MODIFY  comment  VARCHAR(500) CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;')
                    self.cursor.execute(
                        "INSERT INTO taobao1(comment_id,comment,create_date,append_comment,action_sku) VALUES (%s,%s,%s,%s,%s)",
    ...
    def read_30(self):
        ...
        self.cursor.execute('SELECT create_date,comment, append_comment FROM taobao1 ORDER BY create_date DESC ')
        data = self.cursor.fetchall()
        return data

    def jieba_word(self):
        self.cursor.execute('SELECT comment FROM taobao1')
    ...
    def chart(self):
        """生成图片分析svg"""
        self.cursor.execute('SELECT DISTINCT action_sku FROM taobao1')
        cate = self.cursor.fetchall()
        cate_list = []  # 商品类型列表
        for i in cate:
            cate_list.append(i[0])
        self.cursor.execute('SELECT COUNT(id) FROM taobao1')
        num_all = self.cursor.fetchone()[0]  # 查询总数

        cate_num_list = []  # 构造商品类型+统计数列表
        for b in cate_list:
            self.cursor.execute(
                "SELECT COUNT(id) FROM taobao1 WHERE action_sku='%s'" % b)
        ...
        pie_chart.render_to_file('图片.svg')
